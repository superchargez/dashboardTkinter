...
for i in range(4):
    for j in range(4):
        ...
        for k in range(4):
            Label(frames[counter], text=counter, relief=RAISED, borderwidth=2, cursor=cursor[i], width=10,
            bg="light yellow",
            ).grid(row=0,column=k*2+1, sticky="EW", columnspan=2)
            Label(frames[counter], text=rows[k], borderwidth=2, pady=1, relief=RAISED, cursor=cursor[i], width=8,
            bg="light pink",
            ).grid(row=k+1,column=0, sticky="EW", columnspan=1)
            for m in range(4):
